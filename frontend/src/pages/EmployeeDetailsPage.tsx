...
const EmployeeDetailsPage: React.FC = () => {
    ...
    try {
      ...
      const education = baseEducationLevels.map((level) => {
        ...
        return {
          ...
          scorePercentage: existing?.scorePercentage === null || existing?.scorePercentage === undefined ? '' : String(existing.scorePercentage)
        };
      });
  ...
  const handleSave = () => {
    ...
    if (employeeData.education && Array.isArray(employeeData.education)) {
      ...
      const pgYear = parseInt(employeeData.education.find((e: any) => e.level === 'PG')?.year, 10);
      const ugYear = parseInt(employeeData.education.find((e: any) => e.level === 'UG')?.year, 10);
      const hscYear = parseInt(employeeData.education.find((e: any) => e.level === '12th')?.year, 10);

      if (!isNaN(ugYear) && !isNaN(hscYear)) {
        if (hscYear >= ugYear) {
          showWarning('12th Graduation Year must be before UG Graduation Year');
          const hscIndex = employeeData.education.findIndex((e: any) => e.level === '12th');
        ...
        if (ugYear - hscYear < 3) {
          ...
          const ugIndex = employeeData.education.findIndex((e: any) => e.level === 'UG');
      ...
      if (!isNaN(pgYear) && !isNaN(ugYear)) {
        ...
        if (pgYear - ugYear < 2) {
          ...
          const pgIndex = employeeData.education.findIndex((e: any) => e.level === 'PG');
