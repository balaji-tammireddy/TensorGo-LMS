...
export const updateProfile = async (userId: number, profileData: any, requesterRole: string) => {
  ...
  const currentValues = await pool.query('SELECT user_role as role, user_status as status, date_of_birth, date_of_joining, emp_id, email, designation, department, pg_year, ug_year, twelveth_year FROM users WHERE id = $1', [userId]);
  ...
  if (profileData.reportingManagerId !== undefined && (userRole === 'super_admin' || userRole === 'hr')) {
    if (profileData.reportingManagerId) {
      const managerResult = await pool.query('SELECT user_role as role, user_status as status FROM users WHERE id = $1', [profileData.reportingManagerId]);
...
FROM users
WHERE user_role = ANY($1) AND user_status IN ('active', 'on_leave')
`;
